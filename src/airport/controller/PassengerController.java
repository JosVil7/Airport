...
public class PassengerController {
    ...
    public static Responses updatePassenger(String idup, String firstname, String lastname, String year, String month, String day, String phoneCode, String phone, String country) {
        try {
            if (idup == null || idup.trim().isEmpty()) {
                return new Responses("Id must not be empty", Status.BAD_REQUEST);
            }
            if (year == null || year.trim().isEmpty()
                    || month == null || month.trim().isEmpty()
            ...
            long idu = Long.parseLong(idup);
            int yearr = Integer.parseInt(year);
            int monthr = Integer.parseInt(month);
            int dayr = Integer.parseInt(day);
            int phoneCodeu = Integer.parseInt(phoneCode);
            long phoneu = Long.parseLong(phone);
            ...
            if (phoneCode.length() > 3 || phoneCode.length() <= 0) {
                return new Responses("Phone code must be max 3 digits", Status.BAD_REQUEST);
            }
            ...
            // Obtener el pasajero y actualizarlo
            Storage_Passenger storage = Storage_Passenger.getInstance();
            Passenger passenger = storage.getPassenger(idu);
